...
**Totals.** The 👤 figure is the *single talented generalist* bound from each week's Effort Estimate section; the **Totals** row sums those bounds. The 👤🤖 figure is the actual human hours spent that week. Lines-of-code totals (kloc) come from `gather.sh`, which runs `git log --shortstat` on landed commits **excluding**:

1. **Global defaults:** every path under `**/vendor/**` and `**/node_modules/**`.
2. **Fleet metadata:** per-repo globs in this repo’s [`data/line-excludes.yaml`](../data/line-excludes.yaml) (goldens, verdicts, fixture corpora, …). Project repos stay clean — no per-repo config file.

Those trees stay in the project repos for builds and tests; they are omitted from ☲ so cross-repo vendor copies and oracle dumps do not dominate. Commits still count toward ℂ. Binary prebuilts (e.g. `.a` via LFS) already contribute ~0 lines.

**Series restamp (2026-07-25).** Every weekly report and the README Metrics table were remeasured under the global vendor exclude against current default-branch history. Across 26 weeks, ~+3.0M vendor adds were stripped from ☲ (series net ≈ **+3.0M** excl. vendor vs ≈ **+5.6M** raw). Commits and effort/human-hour ranges were not changed. `data/line-excludes.yaml` extras (goldens, verdicts, …) apply from adoption forward; older weeks were not re-stamped for those paths. Open any report for the full derivation — per-project person-days with *why it's hard* rationales, the Diversity Tax list, the What-If-It-Were-One-Person ramp-up table, and the human-hours breakdown.
