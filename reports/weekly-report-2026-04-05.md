...
**432 commits** | **~+37,000 net lines** (excl. vendored/golden/shell.c) | **~90-150 person-days traditional equivalent** | **~25-45x multiplier**
...
- **nostalgia: new TUI git file history browser** — a Go [Bubble Tea](https://github.com/charmbracelet/bubbletea) TUI for browsing file history in git repos with automatic syntax highlighting via [chroma](https://github.com/alecthomas/chroma), terminal background detection via [termenv](https://github.com/muesli/termenv), DAG commit graph with fork/merge lines, topological sort with date tiebreaker, and [go-git](https://github.com/go-git/go-git) replacing shell invocations
...
Continued from last week's scene protocol and H.264 streaming:

- **T54 abandoned, H.264 pivot**: after per-session bgfx hit a dead end (bgfx's internal state is not designed for multi-instance), pivoted to H.264 streaming as the practical path. Full pipeline working: bgfx renders globe → [VideoToolbox](https://developer.apple.com/documentation/videotoolbox) encodes → streams via ged relay → player decodes and displays
- **H.264 end-to-end**: capture integrated into bgfx renderer, `StreamSession` for encode pipeline, `VideoDecoder` for client-side decode. Player stability: input forwarding, ping-safe WebSocket, encoder quality tuning, graceful SIGINT/SIGTERM shutdown
- **Engine ownership** (T55 achieved): `ge::run()` single entry point replacing 140 lines of infrastructure in `main.cpp`. `ge::Context` provides engine-owned SQLite DB handle. `BgfxContext` auto-installs signal handlers — apps don't call it. `ge::Signal` for platform-gated SIGINT/SIGTERM. `GameDb` wraps `ge::Db` instead of raw `sqlite3*`. Game code is now DB-mode-agnostic — engine owns persistence
- **sqlpipe v0.17.0 integration**: liteparser vendored, `db_wrap` from sqlift, `DeviceClass` support
- **Scene protocol** (before pivot): full resource + frame flow through `NetworkBackend` (1,418 resources), `SubmitFrame` serialisation for Metal backend, per-session bgfx, `UniformCacheFrame` serialisation

### [squz/nostalgia](https://github.com/squz/nostalgia) — New TUI Git Browser (14 commits, initial)

New project: a terminal UI for browsing file history in git repositories, built with Go and [Bubble Tea](https://github.com/charmbracelet/bubbletea):

- **File history viewer**: browse the commit history of any file, with diff display and syntax highlighting via [chroma](https://github.com/alecthomas/chroma). Terminal background detection via [termenv](https://github.com/muesli/termenv) for automatic light/dark theme selection
- **DAG commit graph**: visual graph showing branch structure with fork (`\`) and merge (`/`) lines, uniform-width padding for alignment. Topological sort with date tiebreaker for correct DAG interleaving across branches
- **go-git migration**: replaced shell `git` invocations with [go-git](https://github.com/go-git/go-git) library for portable, dependency-free operation. Then added shell fallback for shim/PATH wrapper support
- **All-refs walk**: browse history across all refs (not just HEAD), with HEAD commit highlighted
...
| Total commits | 432 |
| Total lines added | +1,122,715 |
| Total lines removed | -539,192 |
| Net new lines | +583,523 |
...
Rendering git DAG graphs in a terminal is harder than it looks — commits from different branches must interleave correctly, fork and merge points need visual indicators, and the graph width must stay bounded. Nostalgia uses a **topological sort with date tiebreaker** to determine commit ordering, then a column-allocation algorithm that assigns branches to graph columns and renders fork (`\`) and merge (`/`) connectors. The uniform-width padding ensures labels align regardless of graph complexity.
