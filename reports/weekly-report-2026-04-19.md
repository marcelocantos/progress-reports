...
**391 commits** | **~+86,000 net lines** (excl. vendored/generated) | **~140-220 person-days traditional equivalent** | **~35-55x multiplier**
...
- **esfera2 Dawn → bgfx port** (squz/esfera2) — 9 commits porting the geodesic chess sphere + pieces from Dawn/WebGPU onto bgfx, consuming the split ge engine via `add_subdirectory(ge)` + APP_NAME convention, bumping through every ge pin (f1c0408 → 4b231ee → f1c0408 → v0.1.0). +9,657/-9,416 — essentially a rewrite at the rendering layer
- **HMS de-identification audit round 7** (hms) — SQL_AX_MODULE constant covering all 8 AX assessment extension tables (tbl_AX_ReferralInfo, tbl_AX_CCPParticipants, tbl_AX_SCPParticipants, tbl_AX_ReferralActionPlan, tbl_ax_communication, tbl_AX_SCPDocs, tbl_AX_Smoking, tbl_AX_Substance) added to the manifest
...
Geodesic chess sphere + pieces ported from Dawn/WebGPU to bgfx atop ge v0.1.0:

- **Port sphere + pieces onto bgfx**: full re-implementation of the rendering layer. +9,657/-9,416, near-zero net
- **ge migration scaffold**: new build links against bgfx + SessionHost
- **Shaderc pipeline + smoke-test shaders**: vendored shaderc binaries (macOS arm64, Linux x86_64) feeding compiled .bin shaders
- **ge/player target**: desktop H.264 receiver available for desktop testing
- **Integrate ge v0.1.0**: APP_NAME convention adoption, use of `ge/FRAMEWORKS`
- **Pin ge to v0.1.0** after cycling through `f1c0408` → `4b231ee` → `f1c0408` during the week

### [squz/multimaze2](https://github.com/squz/multimaze2) — UI + ge Integration (19 commits)

- **Persistence, mobile builds, and pixel-perfect rendering** (🎯T5): first PR merged
- **Main menu screen** with pack selection and swipe-to-change-level
- **Top bar** with Home button and level title text
- **Schema.h draft** + `Door → DoorDef` rename
- **ge migration** through `4b231ee` → `f1c0408` → v0.1.0 pin
- **ccache wire-up** in build toolchain

### [squz/multimaze](https://github.com/squz/multimaze) — ES1 Oracle (12 commits)

The original iOS ES1 codebase rehabilitated as an oracle for the multimaze2 rewrite:

- **Get MultiMaze building on modern Xcode** after years of neglect
- **Runtime crash fix** — disable intro animation CALayer delegates
- **Pack selection UI visible** after disabling CALayer animations
- **Hide splash overlay elements** that obscured pack selection
- **Fix garbled background textures** on retina displays
- **Force 1x content scale** for GL view
- **Intermittent noisy background fix** in FBO render-to-texture
- **Replace UIAccelerometer with CMMotionManager** for device tilt
- **Clear FBO to transparent black** for correct thumbnail compositing
- **Fix crash on level completion**
- **Lock orientation to portrait**
- **Fix audio on device**

### [squz/yourworld2](https://github.com/squz/yourworld2) — ge Submodule Updates (7 commits)

ge pin updates only — no first-party changes this week. Seven commits cycling through ge versions then pinning to v0.1.0, plus ccache wire-up.
...
Sole Marcelo commit in window:

- **SQL_AX_MODULE constant** covering all 8 AX assessment extension tables (`tbl_AX_ReferralInfo`, `tbl_AX_CCPParticipants`, `tbl_AX_SCPParticipants`, `tbl_AX_ReferralActionPlan`, `tbl_ax_communication`, `tbl_AX_SCPDocs`, `tbl_AX_Smoking`, `tbl_AX_Substance`) added to the PII manifest, closing the coverage gap surfaced by audit round 7
...
| Total commits | 391 |
| Total lines added | +101,134 |
| Total lines removed | -27,586 |
| Net new lines | +73,548 |
