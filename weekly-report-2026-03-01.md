...
- **stock-car-racing Unity 6 upgrade** with comprehensive null-reference lifecycle audit (61 findings across 11 singleton patterns), time-delayed auto-repair feature (5%/hr passive healing per panel), ad SDK conditional compilation (`#if MAXSDK`), and Android 16KB page alignment
...
- **Unity lifecycle audit across 11 singleton patterns** (stock-car-racing): 61 findings where thread-unsafe Firebase/IAP/ad callbacks, missing OnDestroy hooks, and implicit Awake→Start ordering caused null references during scene transitions — each fix must respect the original author's initialisation intent without introducing new ordering dependencies
...
A [Unity](https://unity.com/) mobile racing game undergoing major stabilisation:

- **Unity 6 upgrade** (6000.3.5f2): Full migration to latest LTS, 175+ compiler warnings eliminated, standalone build target fixes, material and rendering configuration updates
- **Null reference lifecycle audit**: 61 findings (8 critical, 16 high, 30 medium, 7 low) across 11 different singleton patterns with no centralised initialisation. Fixes applied in phases: `GameManager.Instance` moved to `Awake()`, `RemoteConfigManager.Apply()` routed to main thread via `ContinueWithOnMainThread()`, duplicate-instance guards on `VFXManager`/`Garage`, `OnDestroy` hooks for singleton cleanup, coroutine reference storage replacing string-based `StopCoroutine`, `DOTween.Kill(this)` in teardown, lambda event subscriptions replaced with named methods
- **Time-delayed auto-repair**: Vehicles heal passively at 5%/hr per panel using real-world time (WorldTimeAPI integration). Celebration window with rotating 3D vehicle preview, reworked as fully dynamic popup requiring no scene setup
- **Ad SDK gating**: `#if MAXSDK` preprocessor directives across 18 files — partner builds without the define compile cleanly with no ad singletons, no event subscriptions, no IAP button
- **Android build fixes**: 16KB page size alignment (Android 15 requirement), minification enabled, versionCode bumped to 209. Facebook adapter downgraded to 6.20.0.0 for compatibility

---

### [minicadesmobile/kart-stars](https://github.com/minicadesmobile/kart-stars) — Code Quality Audit (9 commits)

- **92-finding audit** across 6 severity phases: thread-safety fixes (Firebase callbacks accessing Unity objects off main thread), save data bounds checking, null reference guards on chained `.Instance` access, shared mutable state on `ScriptableObject` powerups, static singleton cleanup on scene unload, delegate leak prevention
- **Singleton pattern fix**: `RuntimeInitializeOnLoadMethod` moved out of generic base class (was causing `applicationIsQuitting` flag to persist across editor play sessions)
- **Unity 2022.3.54f1 update**: Materials, deprecated API migration, project settings
